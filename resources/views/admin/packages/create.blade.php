    ...
    <form method="POST" action="{{ route('admin.packages.store') }}">
        ...
        <div class="mb-4">
            ...
            <select name="tip"
                    ...
                    required>
                <option value="">-- Izaberite tip paketa --</option>
                <option value="M" {{ old('tip') === 'M' ? 'selected' : '' }}>Mesečni</option>
                <option value="G" {{ old('tip') === 'G' ? 'selected' : '' }}>Godišnji</option>
                <option value="D" {{ old('tip') === 'D' ? 'selected' : '' }}>Dnevni</option>
                <option value="T" {{ old('tip') === 'T' ? 'selected' : '' }}>Sa trenerom</option>
            </select>
        </div>

        <div class="mb-4" id="brojTreningaWrapper" style="display:none;">
            <label class="form-label fw-semibold">Broj personalnih treninga</label>
            <input type="number"
                   name="broj_treninga"
                   min="1"
                   value="{{ old('broj_treninga') }}"
                   class="form-control bg-dark text-white border-secondary">
        </div>
